...
| 19 | 2026-09-24 | #49 | Cleanup: `ai.ts` 1650 → 159 lines (six modules), ~6k-token context budget, dead code (knip), GOAL files → `ROADMAP.md` + `docs/history.md` | Deploy healthy, no 5xx; review: pure move, clean import graph |
| 20 | 2026-09-24 | #52 | Confirmed multi-op AI batches apply atomically (migration 051, `apply_ai_operations`) | `ROLLBACK_OK` in production (6 assertions: grants, 5-op batch, replay, stale rollback, inner move failure, cross-farm refusal); nothing persisted. Review: lock-order note corrected, residual risk recorded |
| 21 | 2026-09-24 | #51 | Full CSP kept static (host allowlist; nonces would make all 24 pages dynamic) | Header live; /mapa tiles, charts, chat load with no violations in the logged-in browser |
| 22 | 2026-09-24 | #50 | Frontend revamp: tokens (color = state), Archivo, sidebar shell, every page redesigned by 4 parallel agents; big pages split; health/vaccination date defaults | Hoy, Hacienda, Plan, Mapa, Chat, Finanzas walked in both themes with real data; 2 parity reviews (1 finding, fixed); contrast scan 4656 pairs, 0 < 4.5:1. Phone width not screenshotted |
| 23 | 2026-09-24 | this PR | Polish from the live walk: "Potreros" everywhere, currency filter no longer truncated, 44 px touch targets on coarse pointers | Build emits the coarse-pointer rule |
| 24 | 2026-09-24 | #54 | Browser test of production: Plan del día threw React #418 on every load (isOnline prerendered offline: Node's navigator has no onLine); livestock screens say "potrero" | Before: #418 on each /gestion/plan load. After deploy 17bf295: 3 loads clean; all 17 routes load; ⌘K, potrero expand, map filter, finance periods, theme switch, task create→complete→delete (demo data + audit rows reverted), assistant answer (146 cab., Aftosa 28/9) all pass |
| 29 | 2026-09-24 | #58 | Potreros as a graph: "Usar padrón completo", adjacency from shapes (field-graph.ts), Linderos layer, routes in the move dialog, adjacent-first rotation, LINDEROS in the AI context | "Usar padrón I-995 completo" stored a 9-vertex polygon, label and "Sin dibujar" updated; reverted exactly (triggers off). CampoAI used the linderos context ("no hay linderos dibujados…"). Linderos toggle not exercised: demo farm has no drawn neighbour pair |
| 30 | 2026-09-24 | #59 | Chat history sidebar: conversations per farm (052), ?c= URL, rename/delete, WhatsApp thread; review fix: Back during an answer no longer renders it into another conversation | 052 applied, backfill 44 msgs → 2 conversations, `ROLLBACK_OK`; new conversation titled from the question, Back/URL work, delete cascades (SQL 0 left) |
| 31 | 2026-09-24 | #57 | Tasks as tickets: detail sheet, Tablero (Por hacer / En curso / Hecha), assignee (053); review fixes: status CHECK drop narrowed, stale Deshacer can't revert a newer change | 053 applied, `ROLLBACK_OK`; row opens the ticket; "Mover a → En curso" persisted (SQL in_progress), Deshacer restored pending; audit rows removed |
| 32 | 2026-09-24 | #60 | Agenda calendar view (month grid, day panel, keyboard); open statuses incl. in_progress; "Cerrar" labels | Grid shows 25/9 task, 28/9 Aftosa, 8/10 harvest; day panel opens |
| 33 | 2026-09-24 | #56 | Finanzas charts (flujo, categoría, resultado por sección/cultivo); Métricas fixed/replaced/added | Flujo ago +18.500 / sep −2.940 with tooltip; categories 78/22 %; Métricas sections render; /api/metrics warm 0.4–0.7 s (6.2 s cold) |
| 34 | 2026-09-24 | #61 | Found live: chat showed answers above questions (same created_at, desc+reverse tie) — also in the AI prompt history since before; undo toast kept a stale Deshacer | After deploy: history reads question → answer to the last turn |
