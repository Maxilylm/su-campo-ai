...
export async function POST(req: NextRequest) {
  ...
  const invalidRelation = await Promise.all(
    relationChecks
      .filter(([, id]) => Boolean(id))
      .map(async ([table, id]) => {
        const { data } = await db.from(table).select("id").eq("id", id).eq("farm_id", result.farmId).maybeSingle();
        return data ? null : table;
      })
  );
  const invalidTable = invalidRelation.find(Boolean);
  if (invalidTable) {
  ...
  if (body.type === "compra" && unitCost !== null && unitCost > 0) {
    ...
    // PGRST202 means an older deployment has not applied 010_integrity.sql.
    // Keep that deployment usable, but surface every other failure.
  ...
  if (body.type === "compra" && unitCost !== null && unitCost > 0) {
    ...
    const { error: financialError } = await db.from("financial_transactions").insert({
      farm_id: result.farmId,
      type: "egreso",
    ...
    if (financialError) {
      console.error("Failed to create purchase financial transaction:", financialError.message);
      return NextResponse.json({ error: "La compra se registró, pero no se pudo crear su asiento contable." }, { status: 503 });
    }
  }
