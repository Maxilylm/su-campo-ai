...
import { buildFieldStatus, fieldTotals } from "@/lib/grazing";
...
export async function GET() {
  ...
  const queries = await withTimeout(
    Promise.all([
      // `*` keeps this working before and after the occupancy columns exist.
      db.from("sections").select("*").eq("farm_id", farmId).order("name").limit(MAX_ROWS),
  ...
  const [sections, cattle, crops] = queries;
  ...
  const statuses = buildFieldStatus(sections.data ?? [], cattle.data ?? [], crops.data ?? [], Date.now());
  return NextResponse.json({ sections: statuses, totals: fieldTotals(statuses) });
}
