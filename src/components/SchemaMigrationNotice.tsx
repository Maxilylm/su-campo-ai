...
export function SchemaMigrationNotice({ migrations, compact = false, mode = "migration" }: SchemaMigrationNoticeProps) {
  ...
  return (
    <div className={compact ? "mt-2 text-center" : "mt-3 rounded-md border border-warn-line bg-warn-soft p-3"} role="status" aria-live="polite">
      <div className={compact ? "text-[11px] text-muted-foreground" : "flex flex-wrap items-center justify-between gap-2"}>
        <p className={compact ? "" : "text-sm font-medium text-warn"}>
          {compact
      ...
      {!compact && <p className="mt-1 text-xs text-muted-foreground">
        {diagnostic
          ...
          : compatibilityOnly
          ? "El sistema sigue operativo; aplicalas para habilitar operaciones atómicas y evitar duplicados."
          : "Aplicalas en el SQL Editor de Supabase para habilitar todas las protecciones."}
      </p>}
      ...
      {copyState === "error" && <p className="mt-1 text-[11px] text-warn">No se pudo copiar; seleccioná la lista manualmente.</p>}
    </div>
  );
}
