...
export const config = {
  matcher: [
    {
      // `webmanifest` is exempt so the installability manifest never reaches the
      // auth check. `isPublicPath` also allows /manifest.webmanifest, /sw.js and
      // /robots.txt; the matcher exemption just avoids invoking the proxy at all.
      source: "/((?!_next/static|_next/image|favicon.ico|.*\\.(?:svg|png|jpg|jpeg|gif|webp|webmanifest)$).*)",
      missing: [
