...
| 52 | `052_chat_conversations.sql` | `chat_conversations` (shared per farm, `created_by`, `channel` web/whatsapp with one WhatsApp thread per farm) and `chat_messages.conversation_id` (composite FK with `farm_id`, `ON DELETE CASCADE`); a trigger keeps `updated_at` at the newest message; `backfill_chat_conversations(farm)` moves messages without a conversation into "Conversación anterior" (run once here, safe to re-run). Server-only table. The app falls back to the single shared thread until this is applied. Rollback test: `tests/052_rollback_check.sql` |
| 53 | `053_task_status_and_assignee.sql` | Tasks as tickets: `status` also accepts `in_progress` (still open work everywhere), `assigned_to` (a farm member's auth user id, checked by the API and by a `BEFORE` trigger; removing a member unassigns their tasks), index on `activities(farm_id, metadata->>'record_id')` for the task history. The app hides "En curso" and the assignee until this is applied. Rollback test: `tests/053_rollback_check.sql` |
